...
Renderer::Renderer(Window &parent) : OGLRenderer(parent) {
	...
	for (int i = 0; i < 10; i++)
	{
		CubeRobot* cubeb = new CubeRobot(cube);
		cubeb->SetTransform(Matrix4::Translation(Vector3((i * 100), 35, 0)));
		root->AddChild(cubeb);
	}

	root->SetTransform(root->GetTransform() * Matrix4::Scale(Vector3(1, 10, 1)));
